...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // SECURITY: require auth — either a valid user JWT OR the service-role key
    // (the latter lets DB triggers fan out push via pg_net without a user session).
    const authHeader = req.headers.get('Authorization') || '';
    const isServiceRole =
      authHeader === `Bearer ${serviceRoleKey}` ||
      req.headers.get('x-internal-trigger') === serviceRoleKey;

    const body = await req.json();
    const { user_id, title, message, url: notifUrl, all_users } = body;
