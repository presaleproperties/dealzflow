...
BEGIN
  ...
  rows AS (
    SELECT
      ...
      3                           AS importance,
      jsonb_build_object('note_type', n.note_type) AS metadata
    ...
    SELECT
      ...
      'email',
      CASE
        WHEN e.click_count > 0 THEN 'clicked'
        WHEN e.open_count  > 0 THEN 'opened'
        ELSE 'sent'
      END,
      CASE WHEN e.direction = 'inbound' THEN 'in' ELSE 'out' END,
      COALESCE(e.sent_at, e.created_at),
      COALESCE(NULLIF(e.subject, ''), '(no subject)'),
      CASE WHEN e.direction = 'inbound' THEN 'Email received' ELSE 'Email sent' END,
      LEFT(COALESCE(e.body, ''), 280),
      CASE WHEN e.direction = 'inbound' THEN 7 ELSE 3 END,
      jsonb_build_object('open_count', e.open_count, 'click_count', e.click_count, 'gmail_message_id', e.gmail_message_id)
    ...
    SELECT
      ...
      sh.status,
      NULL,
      ((sh.showing_date::text || ' ' || COALESCE(sh.showing_time::text, '00:00'))::timestamptz),
      'Showing · ' || COALESCE(sh.status, 'scheduled'),
      NULLIF(TRIM(COALESCE(sh.project,'') || CASE WHEN COALESCE(sh.unit,'') <> '' THEN ' · ' || sh.unit ELSE '' END), ''),
      sh.notes,
      6,
      jsonb_build_object('status', sh.status, 'project', sh.project, 'unit', sh.unit)
    ...
    SELECT
      ...
      t.status,
      NULL,
      COALESCE(t.due_date, t.created_at),
      ...
      jsonb_build_object('status', t.status, 'due_date', t.due_date)
