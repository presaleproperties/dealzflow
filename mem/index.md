...
- [SMS Ecosystem v2](mem://features/crm/sms-ecosystem-v1) — Telnyx everywhere: single/bulk/scheduled/Zara SMS+WA all → telnyx-send-message. Inbound webhook fires notification + zara-suggest-reply. get-zara-context returns recentSms. process-scheduled-sms cron every minute drains sms_outbound_queue.
...
- [Templates → Agent Hub Handoff](mem://features/crm/templates-builder-v2) — `/crm/templates` is a read-only mirror; New/Edit/AI buttons deep-link to Presale Agent Hub via `AgentHubLinks` (`src/lib/agentHub.ts`). No in-CRM email builder. Default base `https://app.presaleproperties.com/agent`, override via localStorage `presale.agenthub_url`.
- [Templates Builder v2](mem://features/crm/templates-builder-v2) — Enhanced `TemplateEditor` (full-screen 98vw): SenderIdentityField (locked, no admin spoof), SendTestDialog → `template-send-test` edge fn → bridge-send-email (no contact_id), SyncHistoryList from `crm_template_sync_log`, `useTemplateAutosave` localStorage rescue + dirty pill. Bulk still on crm-mass-send-email.
...
- [Tablet as Scaled Mobile v1](mem://style/tablet-as-scaled-mobile-v1) — Tablets (768–1023) now render the mobile UI scaled up via device-tier tokens. `useIsCompact` flipped to <1024 (== useIsMobile). New `useIsTablet()` helper. Tablet tier: scale 1.14, pad-x 28, gap 26, --m-content-max 720px. .m-page centers + caps width on tablet.
- [Zara Autonomous Replies v0](mem://features/crm/zara-autonomous-replies-v0) — Inbound auto-reply pipeline: `crm_team.is_ai`, `crm_zara_settings` singleton kill switch + quiet hours, `zara_can_send_to(uuid) jsonb` guard (assigned-only + `zara:muted` tag + quiet hours), `zara-reply` edge fn (Lovable AI Gateway, `google/gemini-3-flash-preview`, escalates hot_signal/objection/<0.65), wired into `gmail-sync` + `twilio-sms-webhook`. Audit via `crm_audit_log.action='zara.{blocked,escalation,replied,send_failed,error}'`. Email send is stub-only until Zara's Gmail OAuth is connected.

- [Zara Outbound Planner v1](mem://features/crm/zara-outbound-planner-v1) — Co-pilot mode: AI drafts outbound (cold_nudge/new_lead_welcome/presale_burst/post_showing) into `crm_zara_drafts`, NEVER auto-sends. `/admin/zara/drafts` approval inbox (approve/edit/reject/snooze/mute). Cron `zara-plan-outbound-15m`. Edge fns: `zara-plan-outbound` + `zara-draft-action`.

- [Zara Apple Intelligence v2](mem://style/zara-apple-intelligence-v2) — Editorial polish: new `.zara-rule`/`.zara-meta`/`.zara-link`/`.zara-dot-row`/`.zara-tile`/`.zara-rail`/`.zara-section-head`/`.zara-input`/`.zara-chip`/`.zara-shimmer` utilities. In-lead whisper is text-only links (no icon buttons). ZaraShell uses glass rail. Admin Zara KPIs use `.zara-tile`, not Card.
