...
export function InlineEmailReplyBox({ contact, lastSubject, onOpenFull }: Props) {
  ...
  if (!expanded) {
    return (
      <div className="sticky bottom-0 border-t border-border bg-background/95 backdrop-blur px-3 py-2.5 pb-[calc(env(safe-area-inset-bottom,0px)+max(var(--keyboard-offset,0px),var(--keyboard-inset-bottom,0px))+10px)]">
        <button
          type="button"
          onClick={() => {
            if (!contact.email) {
