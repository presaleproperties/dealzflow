      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={String(step)} onValueChange={v => setStep(Number(v))}>
          <TabsList className="grid grid-cols-4 mb-4">
            ...
            <TabsTrigger value="2" disabled={(!subject || !body) && step < 2}>Recipients</TabsTrigger>
          ...
          <TabsContent value="1" className="space-y-3">
            ...
            <div className="flex justify-end">
              <Button size="sm" disabled={!subject.trim() || !body.trim()} onClick={() => setStep(2)}>Next: Recipients</Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="2" className="space-y-3">
            ...
            <div className="flex justify-end">
              <Button size="sm" disabled={filterType !== 'all' && !filterValue} onClick={() => setStep(3)}>Next: Send</Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="3" className="space-y-4">
            <div className="bg-muted/30 rounded-lg p-4 space-y-2">
              ...
              <p className="text-sm text-foreground">{recipientCount} leads</p>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                className="bg-[hsl(39_67%_55%)] hover:bg-[hsl(39_67%_48%)] text-white"
                disabled={createCampaign.isPending || (scheduleType === 'schedule' && !scheduleDate)}
                onClick={handleSend}
              >
                {createCampaign.isPending ? 'Sending...' : scheduleType === 'now' ? 'Send Now' : 'Schedule'}
              </Button>
            </div>
          </TabsContent>
        </Tabs>
