...
export function AutomationBuilder({ editing, templatePrefill, onClose }: Props) {
  ...
  useEffect(() => {
    if (editing) {
      ...
      setActions([]); setIsActive(false);
    }
    setSelectedNode('trigger');
    setShowAddAction(false);
  }, [editing, templatePrefill]);
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <Input
            ...
            className="border-0 bg-transparent text-base font-semibold h-9 px-0 focus-visible:ring-0 min-w-0 flex-1"
          />
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          ...
          <Button size="sm" onClick={handleSave} disabled={!name.trim() || isSaving}
            className="h-9 gap-1.5 bg-primary hover:bg-primary/90 text-xs">
      ...
      {/* Main builder area — flow canvas + config panel side by side */}
      <div className="flex flex-col lg:flex-row gap-0 rounded-xl border border-border/50 bg-card/30 overflow-hidden" style={{ minHeight: 'calc(100dvh - 260px)' }}>
        {/* Flow canvas */}
        <div className="flex-1 bg-muted/5 overflow-y-auto">
          <div className="p-6 sm:p-8 flex flex-col items-center">
            ...
            <div
              ...
              className={`w-full max-w-[320px] rounded-xl border-2 cursor-pointer transition-all ${
            ...
            {showAddAction ? (
              <div className="w-full max-w-[320px] rounded-xl border-2 border-dashed border-primary/40 bg-primary/5 p-4">
                <p className="text-xs font-medium text-foreground mb-3">Add an action</p>
                  ...
                  {ACTION_TYPES.map(at => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center gap-2 p-2.5 rounded-lg hover:bg-card/80 transition-colors text-left border border-transparent hover:border-border/50"
                      >
                ...
                <Button variant="ghost" size="sm" onClick={() => setShowAddAction(false)} className="w-full mt-3 h-8 text-xs">
                  Cancel
                </Button>
              </div>
            ) : (
              <button
                ...
                className="h-11 w-11 rounded-full border-2 border-dashed border-border hover:border-primary/50 flex items-center justify-center hover:bg-primary/5 transition-all group"
              >
        ...
        {/* Config panel */}
        <div className="w-full lg:w-[340px] border-t lg:border-t-0 lg:border-l border-border/50 bg-card/50 flex flex-col shrink-0">
