    ...
    <Draggable draggableId={contact.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`bg-card rounded-lg border border-border p-3 mb-2 shadow-sm cursor-grab transition-shadow ${snapshot.isDragging ? 'shadow-lg ring-2 ring-primary/20' : 'hover:shadow-md'}`}
        >
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <Select value={filterAgent} onValueChange={setFilterAgent}>
          <SelectTrigger className="h-9 w-[170px] text-xs"><SelectValue placeholder="All Agents" /></SelectTrigger>
          <SelectContent>
          ...
          <div className="flex-1 overflow-x-auto pb-4">
            <div className="flex gap-3 min-w-max h-full">
              {LEAD_STATUSES.map(stage => (
                <div
                  key={stage}
                  className="flex flex-col w-[240px] rounded-xl border border-border/50 flex-shrink-0"
