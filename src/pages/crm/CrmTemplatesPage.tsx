...
import { Plus, Search, Star, Copy, Trash2, Pencil, Code, ArrowUpDown, Globe, Send } from 'lucide-react';
...
import { CampaignComposer } from '@/components/crm/templates/CampaignComposer';
import { CampaignsTab } from '@/components/crm/templates/CampaignsTab';
...
export default function CrmTemplatesPage() {
  ...
  const openCampaign = (tpl: EmailTemplate, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  return (
    <div className="space-y-4">
      {/* Sync Status Bar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-lg bg-card/60 border border-border/40 px-4 py-2.5 gap-2">
                ...
                >
                  ...
                  <div className="p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button className="shrink-0 mt-0.5" onClick={(e) => handleToggleFav(tpl, e)}>
                  ...
                  <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="secondary" size="sm" className="h-7 w-7 p-0" onClick={(e) => { e.stopPropagation(); openEditor(tpl); }} title="Edit">
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="secondary" size="sm" className="h-7 w-7 p-0" onClick={(e) => openCampaign(tpl, e)} title="Send as Campaign">
                      <Send className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="secondary" size="sm" className="h-7 w-7 p-0" onClick={(e) => handleCopyHtml(tpl, e)} title="Copy HTML">
                      <Code className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="secondary" size="sm" className="h-7 w-7 p-0" onClick={(e) => handleDuplicate(tpl, e)} title="Duplicate">
                      <Copy className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="secondary" size="sm" className="h-7 w-7 p-0 hover:bg-destructive hover:text-destructive-foreground" onClick={(e) => handleDelete(tpl.id, e)} title="Archive">
      ...
      {campaignTemplate && (
        <CampaignComposer
          ...
          open={!!campaignTemplate}
          onOpenChange={(open) => { if (!open) setCampaignTemplate(null); }}
        />
      )}
    </div>
  );
